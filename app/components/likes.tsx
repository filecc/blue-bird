...
export default function Likes({ tweet }: { tweet: TweetWithAuthor }) {
  ...
  return (
    <button onClick={handleLikes} className="flex items-center gap-2 group">
      <svg
      ...
      <span className={`text-sm group-hover:text-red-600  ${tweet.user_has_liked_tweet ? 'text-red-600' : 'text-gray-500'}`}>
        {tweet.likes}
      </span>
